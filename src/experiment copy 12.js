...
async function runExperiment() {
  ...
  function createTypingTrial(pair, trialId) {
    ...
    return {
      ...
      on_load: () => {
        ...
        const endTrial = () => {
          ...
          jsPsych.finishTrial({
            trialId: trialId, // Add unique identifier
      ...
      on_finish: (data) => {
        // Directly write the trial data without any flags
        jsPsych.data.write({
  ...
  // Create trials for each pair in the sublist with unique trial IDs
  sublist.forEach(([pair1, pair2], index) => {
    timeline.push(createTypingTrial(pair1, `trial-${index}-1`));
    timeline.push(createTypingTrial(pair2, `trial-${index}-2`));
