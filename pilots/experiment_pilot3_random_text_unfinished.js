...
function generateTextWithBigrams(bigram1, bigram2, ncharacters) {
  ...
  for (let bigram of bigrams) {
    while (bigram.count < 3) {
      ...
      text = text.slice(0, position) + bigram.bigram + text.slice(position + 2);
...
function createTypingTrial(bigram1, bigram2, trialId) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      for (let bigram of bigrams) {
        if (text.substr(i, 2) === bigram.bigram) {
          ...
          styledText += `<span style="${style}">${text[i]}</span><span style="${style}">${text[i+1]}</span>`;
          i++; // Skip the next character as we've already added it
          break;
        }
      }
      if (!style) {
        styledText += `<span>${text[i]}</span>`;
      }
    }
    return styledText;
  }
  ...
  function handleKeyPress(event) {
    ...
    if (typedKey === expectedKey) {
      ...
      updateLetterColors(typedSequence.length - 1);
      keyData.push({
      ...
      if (typedSequence === text) {
        setTimeout(() => {
          jsPsych.finishTrial({
            ...
            fullText: text,
            styledText: document.getElementById('text-to-type').innerHTML,
            trialId: trialId
          });
        }, 500);
      }
  ...
  function updateLetterColors(index) {
    const letterSpans = document.querySelectorAll('#text-to-type span');
    if (letterSpans[index]) {
      letterSpans[index].style.color = 'gray';
    }
  }
...
// Comfort choice trial with buttons and slider
function createComfortChoiceTrial(bigram1, bigram2, trialId) {
  return {
    type: htmlKeyboardResponse,
    stimulus: `
      <div id="comfort-choice-container">
        <div id="typed-text-container"></div>
        <p style="font-size: 28px;">Which letter pair was easier to type?</p>
        <div id="bigram-buttons">
          <button class="jspsych-btn comfort-choice-button" data-bigram="${bigram1}" style="color: #0072B2;">${bigram1}</button>
          <button class="jspsych-btn comfort-choice-button" data-bigram="${bigram2}" style="color: #D55E00;">${bigram2}</button>
        </div>
        <div id="slider-container" style="display: none;">
          <p id="slider-question" style="font-size: 24px;"></p>
          <div id="custom-slider" style="width: 500px; margin: 20px auto;">
            <input type="range" min="0" max="100" value="50" style="width: 100%; -webkit-appearance: none; background: #d3d3d3; outline: none; opacity: 0.7; transition: opacity .2s;">
            <div style="display: flex; justify-content: space-between; margin-top: 5px;">
              <span>slightly easier</span>
              <span>much easier</span>
            </div>
          </div>
          <button id="submit-button" class="jspsych-btn" style="display: none;">Submit</button>
        </div>
      </div>
    `,
    choices: "NO_KEYS",
    on_load: function() {
      let chosenBigram = null;
      const bigramButtons = document.querySelectorAll('.comfort-choice-button');
      const sliderContainer = document.getElementById('slider-container');
      const slider = document.querySelector('#custom-slider input');
      const sliderQuestion = document.getElementById('slider-question');
      const submitButton = document.getElementById('submit-button');

      // Display the typed text from the previous trial
      const lastTypingTrial = jsPsych.data.get().last(trial => trial.task === 'typing').values()[0];
      if (lastTypingTrial && lastTypingTrial.styledText) {
        document.getElementById('typed-text-container').innerHTML = `
          <p id="typed-text" style="font-size: 24px; letter-spacing: 2px;">${lastTypingTrial.styledText}</p>
        `;
      } else {
        console.error('No styled text found from the last typing trial');
      }

      // Disable the slider initially
      slider.disabled = true;

      bigramButtons.forEach(button => {
        button.addEventListener('click', function() {
          chosenBigram = this.dataset.bigram;
          const unchosenBigram = chosenBigram === bigram1 ? bigram2 : bigram1;
          sliderQuestion.innerHTML = `How much easier was <span style="color: ${chosenBigram === bigram1 ? '#0072B2' : '#D55E00'}; font-weight: bold;">${chosenBigram}</span> to type than <span style="color: ${unchosenBigram === bigram1 ? '#0072B2' : '#D55E00'}; font-weight: bold;">${unchosenBigram}</span>?`;
          sliderContainer.style.display = 'block';
          bigramButtons.forEach(btn => btn.disabled = true);
      ...
      slider.addEventListener('input', function() {
        this.style.background = `linear-gradient(to right, #3498db 0%, #3498db ${this.value}%, #d3d3d3 ${this.value}%, #d3d3d3 100%)`;
      });

      submitButton.addEventListener('click', function() {
        if (chosenBigram && slider.value !== '50') {
          jsPsych.finishTrial({
            task: 'comfort_choice',
            chosenBigram: chosenBigram,
            unchosenBigram: chosenBigram === bigram1 ? bigram2 : bigram1,
            sliderResponse: parseInt(slider.value),
            trialId: trialId
          });
        }
      });
...
function createCombinedTrial(bigram1, bigram2, trialId) {
  return {
    timeline: [
      createTypingTrial(bigram1, bigram2, trialId),
      createComfortChoiceTrial(bigram1, bigram2, trialId)
    ]
  };
}
  ...
  if (!experimentConfig.practiceOnly) {
    experimentTimeline.timeline.push(
      ...
      ...processedMainPairs.flatMap(([bigram1, bigram2], index) => [
        createCombinedTrial(bigram1, bigram2, `main-trial-${index + 1}`)
      ])
    );
  }
